...
/*
  A Token is a string of input characters that represent a specific class of C++
  grammar. For example a token can be an identifier, keyword, digit, symbol, or
  etc.
*/
class Token {
private:
...
public:
  /*
    Default constructor creates a Token without a next or previous token and an
    empty string value
  */
  ...
  */
  Token(const string &stringRep)
      : next(NULL), prev(NULL), stringRep(stringRep) {}

  /*
    Returns the next token in the list. A NULL value represents the end of the
    list
  */
  ...
  /*
    ...
    @param next - the next token in the list, a NULL value represents the end of
    the list
*/
  void setNext(Token *next) { this->next = next; }

  /*
    Returns the previous token in the list.  A NULL value represents the
...
Use the given Token ptr for the previous Token in the list
    @param prev the previous token in the list, A NULL value represents the
beginning of the list
  */
...
public:
  ...
  */
  ...
  /*
    Returns a pointer to the fisrt token in the list. A NULL value represents an
    empty list.
  */
  Token *getFirst() const { return head; }

  /*
    Returns a pointer to the last token in the list. A NULL value represents an
    empty list.
  */
  Token *getLast() const { return tail; }

  /*
      Returns the next token to be read from the list. A NULL value represents
     an empty list or end of list
  */
  Token *getNextInList() {
...
*/
  ...
  commands_t stateCommands[78] = {IGNORE,
                                  STORE,
                                  ACCEPT,
                                  ACCEPT,
                                  ACCEPT_BACK1,
  ...
  /*
    Takes the buffer of characters adds it to the list of tokens.  Then
    increases the offset one position
  */
  void addToken();
...
public:
  ...
  */
  ...
  /*
    My setString implementation takes a string and tokenizes it.  Each token is
    first added to a list inside the Tokenizer class. When all tokens are added
    from a line they can be retrieved by the getNextToken function one-by-one.
    @param str the string to be tokenized.  Not allowed to be NULL.
  */
  void setString(string *str);

  /*
   Returns true if all possible tokens have been extracted from the internal
   token list
  */
